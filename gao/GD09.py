...
def show_layouts(layouts: [tuple],axis_range):
    ...
    for bounds in layouts:
        ...
        ax.plot([min_x, max_x, max_x, min_x, min_x], [min_y, min_y, max_y, max_y, min_y], color='black', linewidth=0.5)
...
def show_car_and_pillar(car,pillar,axis_range):
    ...
    for bounds in pillar:
        min_x,min_y,max_x,max_y=bounds
        ax.plot([min_x, max_x, max_x, min_x, min_x], [min_y, min_y, max_y, max_y, min_y], color='red', linewidth=0.5)
    ax.axhline(0, color='black', linewidth=0.5)
    ax.axvline(0, color='black', linewidth=0.5)
    # 保持xy比例一致
    ax.set_aspect('equal', adjustable='box')
...
def expand_line(line:LineString,polygon:Polygon):
    ...
    start_y=min_y
    end_y=max_y
    for x in np.arange(start_x,end_x,CFG.min_size):
...
def random_cut_polygon(polygon:Polygon):
    ...
    while polygon.area>origin_area*CFG.min_area_rate:
        ...
        if polygon.contains(cur_point):
            ...
            cur_polygon=Polygon([(start_x,start_y),(end_x,start_y),(end_x,end_y),(start_x,end_y)])
...
def inner_layout(polygon_bounds: tuple, layout_type: int):
    ...
    min_x, min_y, max_x, max_y = min_x+CFG.road_width / \
        2, min_y+CFG.road_width/2, max_x-CFG.road_width/2, max_y-CFG.road_width/2
    count_num_i = 0
    i = min_x
        ...
        while j < max_y-car_length:
            ...
            if count_num_i %4== 1 and count_num_j%2 ==1:
                if count_num_j==1 and count_num_i==1:
                    i+=CFG.pillar_width
                pillar_bounds_list.append((i-CFG.pillar_width,j,i,j+CFG.pillar_length))
                if j+car_length<max_y:
                    pillar_bounds_list.append((i-CFG.pillar_width,j+car_length-CFG.pillar_length/2,i,j+car_length+CFG.pillar_length/2))
                if j+car_length*2<max_y:
                    pillar_bounds_list.append((i-CFG.pillar_width,j+car_length*2-CFG.pillar_length,i,j+car_length*2))
            car_bounds_list.append(
                [i,j,i+car_width,j+car_length])
            if (count_num_i %4== 0 or i+car_width*2>max_x) and count_num_j%2==1 :
                i+=CFG.pillar_width
                pillar_bounds_list.append(
                    (i+car_width-CFG.pillar_width,j,i+car_width,j+CFG.pillar_length))
                if j+car_length<max_y:
                    pillar_bounds_list.append(
                        (i+car_width-CFG.pillar_width,j+car_length-CFG.pillar_length/2,i+car_width,j+car_length+CFG.pillar_length/2))
                if j+car_length*2<max_y:
                    pillar_bounds_list.append(
                        (i+car_width-CFG.pillar_width,j+car_length*2-CFG.pillar_length,i+car_width,j+car_length*2))
                i-=CFG.pillar_width
            if count_num_j%2==0:
                j+=CFG.road_width
            j+=car_length
...
def outer_layout(polygon):
    ...
    for i in range(len(ex_points)):
        ...
        if not concavity_cur and not concavity_next:
            if isclose(cur_point[0], next_point[0]):
                ...
                for j in range_iterator(cur_point[1], next_point[1]+limit*directiony, car_bounds[0]):
                    min_x,min_y,max_x,max_y =cur_point[0],j,cur_point[0]+car_bounds[1]*directionx,j+car_bounds[0]*directiony
                ...
                for j in range_iterator(cur_point[0], next_point[0]+limit*directionx, car_bounds[0]):
                    min_x,min_y,max_x,max_y =j,cur_point[1],j+car_bounds[0]*directionx,cur_point[1]+car_bounds[1]*directiony
                    min_x,max_x=min(min_x,max_x),max(min_x,max_x)
                    min_y,max_y=min(min_y,max_y),max(min_y,max_y)
                    car_layouts.append((min_x,min_y,max_x,max_y))
        if concavity_cur and not concavity_next:
            if isclose(cur_point[0], next_point[0]):
                directionx = 1 if line2[0] > 0 else -1
                directiony = 1 if line1[1] > 0 else -1
                for j in range_iterator(cur_point[1]+car_bounds[1]*directiony, next_point[1]+limit*directiony, car_bounds[0]):
                    min_x,min_y,max_x,max_y=cur_point[0],j,cur_point[0]+car_bounds[1]*directionx,j+car_bounds[0]*directiony
                    min_x,max_x=min(min_x,max_x),max(min_x,max_x)
                    min_y,max_y=min(min_y,max_y),max(min_y,max_y)
                    car_layouts.append((min_x,min_y,max_x,max_y))
                ...
                directionx = 1 if line1[0] > 0 else -1
                directiony = 1 if line2[1] > 0 else -1
                for j in range_iterator(cur_point[0]+car_bounds[1]*directionx, next_point[0]+limit*directionx, car_bounds[0]):
                    min_x,min_y,max_x,max_y=j,cur_point[1],j+car_bounds[0]*directionx,cur_point[1]+car_bounds[1]*directiony
                    min_x,max_x=min(min_x,max_x),max(min_x,max_x)
                    min_y,max_y=min(min_y,max_y),max(min_y,max_y)
                    car_layouts.append((min_x,min_y,max_x,max_y))
    ...
    for car_layout in car_layouts:
        ...
        cur_polygon = cur_polygon.union(Polygon([(min_x,min_y),(max_x,min_y),(max_x,max_y),(min_x,max_y)]))
...
polygon_input=polygon_input=read_input('GD09-input.txt')
min_x,min_y,max_x,max_y = polygon_input.bounds
show_polygon(polygon_input)
polygon_input_buffer=buffer(polygon_input,-CFG.car_length-CFG.road_width/2,join_style="mitre")
show_polygon(polygon_input_buffer)
best_num=0
best_car_layouts=[]
best_pillar_layouts=[]
for i in range(CFG.rand_times):
    polygon_bounds_list=random_cut_polygon(polygon_input_buffer)
    inner_layouts=[]
    pillar_layouts=[]
    for polygon_bounds in polygon_bounds_list:
        car_bounds_list,pillar_bounds_list=inner_layout(polygon_bounds,0)
