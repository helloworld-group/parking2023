...
# 提取多边形的外轮廓
    ...
    exterior_gdf = gpd.GeoSeries([exterior])
    # 绘制多边形和它的外轮廓
    fig, ax = plt.subplots()
    # gdf.plot(ax=ax, color='blue', alpha=0.5)
    exterior_gdf.plot(ax=ax, color='red')
...
def show_car_and_pillar(layout,car, pillar, obstacles, axis_range,img_name=None):
    ...
    # plot the outline of the parking sapce
...
def outer_layout(polygon, obstacles: List[Polygon] = []):
    ...
    for i in range(len(ex_points)):
        ...
        # the two lines describes the shape of the current corner
        ...
        if not concavity_cur and not concavity_next:
            if isclose(cur_point[0], next_point[0]):
                ...
                # the directions decides how we generate the parking lot from the original point
...
def main():
    ...
    # show_polygon_exterior(polygon_input)
    ...
    polygon_input_obstacle=polygon_input
    for obstalce in obstacles:
